...
namespace Rhema.Data
{
    ...
    public abstract class DapperRepositoryBase 
    {
        ...
        public virtual async Task<TEntity> GetByIdSql<TEntity>( object param = null, string returnFields = null, string orderBy = null, IDbTransaction tran = null, int? commandTimeout = null
            , bool useTransaction = false)
        {
            SqlMapperSetTypeMap<TEntity>();
            ...
            try
            {
                ...
                if (useTransaction)
                {
                    ...
                    result = await session.Connection.GetByIdSql<TEntity>(SqlBuilder, returnFields, param , session.Transaction, commandTimeout);

                    session.Commit();
                }
                else
                {
                    result = await session.Connection.GetByIdSql<TEntity>(SqlBuilder, returnFields, param, null, commandTimeout);
